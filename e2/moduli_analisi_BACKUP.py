...
def fourier(dati,inquinante):
    nyquist = 0.5
    coef_fourier = fft.rfft(dati[inquinante].values)
    # spettro_pot = np.absolute(coef_fourier[:len(coef_fourier)//2])**2 # eventuale coef. 0
    spettro_pot = np.absolute(coef_fourier[1:len(coef_fourier)//2])**2 
    freq_fourier = nyquist*fft.rfftfreq(len(coef_fourier), d=1)
    freq_plot = freq_fourier[1:len(coef_fourier)//2]
    # freq_plot = freq_fourier[:len(coef_fourier)//2] #eventuale coef. 0
    return freq_plot,spettro_pot 
...
def spettro_stati():
    ...
    for j in range(len(stati)):
        plt.figure(j+1)
        for i in range(len(inquinanti)):
            freq_plot,spettro_pot = fourier(nazione,inquinanti[i],etichette[i])
            plt.plot(freq_plot,spettro_pot, label=etichette[i])
            indice_cmax = np.argmax(spettro_pot) #+1 !!
            plt.plot(freq_plot[indice_cmax],spettro_pot[indice_cmax],'.',label='massimo')
            print('\n\n'+etichette[i]+'\nMassimo {:f}\nFreq {:f}\nPeriodo aprox. int. '.format( spettro_pot[indice_cmax], freq_plot[indice_cmax])+'{:d}'.format(int(1/freq_plot[indice_cmax]))  ) #!!
...
# !!!!!
    ...
    # cops                           
    # cofreq                         freq_fourier
    # cofreq[1:len(coft)//2]      freq_fourier[1:len(coef_fourier)//2]          freq_plot
    # cops[1:len(coft)//2]             spettro_pot